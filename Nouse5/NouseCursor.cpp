...
CNouseCursor::CNouseCursor(void) :
...
bool CNouseCursor::create(int nWidth, ETranslucenceType eTranslucenceType) {
...
void CNouseCursor::show() {
	if(m_hwnd == NULL) return;
	ShowWindow(m_hwnd, SW_SHOW);
	m_bCursorVisible = true;
}

void CNouseCursor::hide() {
	if(m_hwnd == NULL) return;
...
void CNouseCursor::redraw(CState* pState, CPVS* pPVS) {
	...
	switch(pState->m_eWhoAmI) {
		...
		case STATE_CURSOR:
			...
			if (pStateCursor->m_bTrialExpired)
			{
				strcpy(m_strClick, "No Click!");//dima demo for click change
				m_nColor[0] = 0;
				m_nColor[1] = 255;
				m_nColor[2] = 0;
			}
	...
	if(pState->m_eWhoAmI == STATE_READY) {
		...
		if(eCursorMode == WAITING_TO_CLICK) {
			...
			pvu::drawRectangle(m_pImc, rectClick, cvScalar(m_nColor[0], m_nColor[1], m_nColor[2]), 4);
		}
...
void CNouseCursor::setPos(PVI_POINT pt) {
	if(m_hwnd == NULL) return;
...
ATOM CNouseCursor::registerWindowClass(HMODULE hInstance) {
	WNDCLASSA wc;
...
LRESULT CALLBACK CNouseCursor::customWinProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	switch(uMsg) {
		case WM_PAINT:
			//m_pObj->redraw();
			break;
...
void CNouseCursor::clearStrings(void)
{
