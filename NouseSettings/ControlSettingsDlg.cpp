...
BEGIN_MESSAGE_MAP(CControlSettingsDlg, CDialog)
	...
	ON_EN_UPDATE(IDC__ACTIVATE_ASSUMES_HEADWIDTH_EDIT, &CControlSettingsDlg::OnUpdateActiveHeadwidth)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
BOOL CControlSettingsDlg::OnInitDialog()
{
	...
	m_ToolTip.AddTool(pNumericText, ACTIVATE_ASSUMES_HEADWIDTH_TOOL_TIP);
	m_ToolTip.Activate(TRUE);
	pNumericText = (CEdit*)GetDlgItem(IDC_NO_CHANGEBACK_SECONDS_EDIT);
	pNumericText->SetLimitText(3);
	m_ToolTip.AddTool(pNumericText, NO_CHANGE_BACK_TO_RESET_TOOL_TIP);
	pNumericText = (CEdit*)GetDlgItem(IDC_BIG_MOTION_SIZE_EDIT);
	pNumericText->SetLimitText(3);		
	m_ToolTip.AddTool(pNumericText, NO_CHANGE_BACK_TO_RESET_TOOL_TIP);
	pNumericText = (CEdit*)GetDlgItem(IDC_NO_MOTION_BACK_EDIT);
	pNumericText->SetLimitText(3);
	m_ToolTip.AddTool(pNumericText, NO_MOTION_BACK_TO_RESET_TOOL_TIP);
	pNumericText = (CEdit*)GetDlgItem(IDC_JOYSTICK_NEUTRAL_ZONE_EDIT);
	pNumericText->SetLimitText(3);
	m_ToolTip.AddTool(pNumericText, JOYSTICK_NEUTRAL_ZONE_TOOL_TIP);
	m_ToolTip.AddTool(GetDlgItem(IDC_CLICK_ON_COMMAND_CHECK), CLICK_ON_COMMAND_TOOL_TIP);
	m_ToolTip.AddTool(GetDlgItem(IDC_USE_MOUTH_CLICK_CHECK), USE_MOUTH_CLICK_TOOL_TIP);
	m_ToolTip.AddTool(GetDlgItem(IDC_CALIBRATE_AUTOMATICALLY_CHECK), CALIBRATE_AUTOMATICALLY_TOOL_TIP);
	m_ToolTip.AddTool(GetDlgItem(IDC_BIG_MOTION_SIZE_EDIT), BIG_MOTION_SIZE_TOOL_TIP);
...
void CControlSettingsDlg::OnUpdateActiveHeadwidth()
{
	CString strText;

	CWnd* pWnd = GetDlgItem(IDC__ACTIVATE_ASSUMES_HEADWIDTH_EDIT);
	pWnd->GetWindowText(strText);

	strText.Trim();
	for( int i= 0; i < strText.GetLength();i++)
	{
		int nChar = strText.GetAt(i);
		if (nChar < 46 && nChar > 57 && nChar != 47)
		{
			strText.Remove(strText.GetAt(i));
